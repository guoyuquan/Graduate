...
int Sock::send_msg(const std::string &msg)
{
    ...
    std::cout<<buf+sizeof(rpp)<<std::endl;
    buf[sizeof(rpp)+msg.size()]=0;
...
int Action::login(const char * msg, string &fip)
{
    ...
    //std::cout<<account<<pwd<<client<<ip<<std::endl;
    account=account.substr(8);
    pwd=pwd.substr(9);
    DBConn dbconn;
...
int proc_msg(const char * msg, string & param, int soc)
{
    ...
    {
        case LOG_IN :
            //std::cout<<msg+sizeof(struct rp2p)<<std::endl;
        ...
        case LOG_OUT :
            ...
            ret+=-13;
            sock.send_msg("Log out succeeds");
            break;
        case LOGGED :
